...
export default function DocumentPanel({
    ...
    // ── Local Zoom State (Independent per panel!) ──
    ...
    useEffect(() => {
        if (scrollTargetGlobal !== null && scrollTargetGlobal !== undefined && numPages > 0) {
            ...
            if (el) {
                ...
                setTimeout(() => setLocalHover(null), 1500); // Briefly flash highlight
            }
    ...
    return (
        <div
            className={`flex-1 flex flex-col overflow-hidden ${isSidePanel ? 'bg-[#F8F7F4] border-l shadow-2xl z-10' : 'bg-[#F8F7F4] border-r'}`}>

            {/* Conditional Header for Reference Panel */}
            {isSidePanel && (
                <div
                    className="shrink-0 sticky top-0 z-20 bg-parchment/95 backdrop-blur-md border-b border-border/40 px-6 py-4 flex items-center justify-between">
                    <div className="flex flex-col">
                        <span className="text-[10px] text-accent font-bold tracking-widest uppercase mb-1">Reference Document</span>
                        <h2 className="font-playfair text-lg font-semibold text-ink truncate">{doc.name.replace(/\.pdf$/i, '')}</h2>
                    </div>
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-10 space-y-24">
                <Document
                    file={pdfUrl}
                    onLoadSuccess={(pdf) => {
                ...
                >
                    <div className="max-w-[1700px] mx-auto">
                        {Array.from({length: numPages}).map((_, i) => {
                            ...
                            return (
                                <div key={i} className="flex gap-12 items-start h-[780px] mb-24">
                                    ...
                                    <div className="shrink-0 flex flex-col">
                                        ...
                                        <div
                                            ...
                                            >
                                                <Page pageNumber={i + 1} width={pdfWidth} renderTextLayer={false}
                                    ...
                                    <div className="flex-1 flex flex-col h-[720px] mt-6">
                                        <div className="flex items-center justify-between mb-2">
                                            <span
                                                className="text-[10px] font-bold text-muted/40 uppercase font-lora tracking-wider">Transcription</span>
                                            {!isBulkEditing && lines.length > 0 && (
                                                <div className="flex items-center gap-4">
                                                    ...
                                                    <button onClick={() => {
                                                        if (confirm("Clear all transcriptions for this page?")) deleteAllOnPage(doc.id!, i);
                                                    }}
                                        ...
                                        <div
                                            ...
                                            {isBulkEditing ? (
                                                <BulkEditor
                                                    ...
                                                    onSave={(newLines: any[]) => {
                                                        updatePageTranscriptions(doc.id!, i, newLines);
                                                    ...
                                                    {lines.map((line, idx) => {
                                                        ...
                                                        return (
                                                            <SentenceRow
                                                                ...
                                                                onSave={(val) => updateTranscription(doc.id!, i, idx, val)}
                                                                onDelete={() => deleteTranscriptionLine(doc.id!, i, idx)}
                                                                onZoomRequest={() => {
